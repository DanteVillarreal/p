...
01/05/24 - Code-commented functions due to minimums. I will go for XLM. Why? AVAX doesnt have reliable speeds. XRP keeps appearing and disappearing from Gemini, so unreliable. FTM isnt on coinbase. XLM has consistent transaction speeds, and they are low and is on bitstamp, coinbase, kraken, not on gemini. not mad about that. I dont like it, but its part of the game so ill leave it on solana. As a result, I need to add XLM websocket client and parsing, then do rest API client for those. I got this!
